...
class ManoManoFeedGenerator:
    """
    Generate rows matching ManoMano format with ALL REQUIRED FIXES:
    ✓ Changed 'category' to 'mm_category'
    ✓ Changed 'picture_X' to 'image_X'
    ✓ Changed 'parent_sku' to 'Parent_SKU'
    ✓ Added unit columns: width_unit, height_unit, length_unit, weight_unit
    ✓ Reordered columns: width, width_unit, height, height_unit, length, length_unit, weight, weight_unit
    """
    COLS = [
    ...
    def create_row(self, product: Dict, info: Dict, images: List, quantity: int,
        ...
        return {
            ...
            'brand': "Autres",
            'mm_category': "20204",  # ✓ FIXED: Changed from 'category' to 'mm_category'
            'title': (info.get('name', 'Product') or 'Product')[:150],
            'description': (info.get('description', '') or '')[:5000],
            'image_1': imgs[0],  # ✓ FIXED: Changed from 'picture_1' to 'image_1'
            'image_2': imgs[1],  # ✓ FIXED: Changed from 'picture_2' to 'image_2'
            'image_3': imgs[2],  # ✓ FIXED: Changed from 'picture_3' to 'image_3'
            'image_4': imgs[3],  # ✓ FIXED: Changed from 'picture_4' to 'image_4'
            'image_5': imgs[4],  # ✓ FIXED: Changed from 'picture_5' to 'image_5'
            ...
            'width': round(width, 2),
            'width_unit': 'cm',  # ✓ FIXED: Added unit column
            'height': round(height, 2),
            'height_unit': 'cm',  # ✓ FIXED: Added unit column
            'length': round(length, 2),
            'length_unit': 'cm',  # ✓ FIXED: Added unit column
            'weight': round(weight, 2),
            'weight_unit': 'kg',  # ✓ FIXED: Added unit column
            'DisplayWeight': round(weight, 2),
            'volume': round(volume, 2),
            'Parent_SKU': str(parent_sku or product.get('id', '') or ''),  # ✓ FIXED: Changed from 'parent_sku' to 'Parent_SKU'
...
<body>
 ...
 <table>
   <tr><th>Image</th><th>SKU</th><th>Title</th><th>EAN</th><th>Price</th><th>Stock</th></tr>"""
            for row in data[:50]:
                img = f'<img src="{row["image_1"]}" alt="Product">' if row.get('image_1') else 'No image'  # ✓ FIXED: Changed from 'picture_1' to 'image_1'
...
def main():
    print("🚀 MANOMANO FEED GENERATOR (BigBuy) - FIXED VERSION")
    print("=" * 70)
    ...
    print("\n✓ ALL MANOMANO FORMAT FIXES APPLIED:")
    print("  • Changed 'category' → 'mm_category'")
    print("  • Changed 'picture_X' → 'image_X'")
    print("  • Changed 'parent_sku' → 'Parent_SKU'")
    print("  • Added unit columns: width_unit, height_unit, length_unit, weight_unit")
